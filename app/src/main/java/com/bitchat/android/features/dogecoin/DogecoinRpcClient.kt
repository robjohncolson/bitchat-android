...
class DogecoinRpcClient(
    ...
    private fun normalizedRawTransactionHex(rawTransactionHex: String): String {
        ...
        validateRawTransactionShape(rawBytes, DogecoinProtocol.MIN_STANDARD_OUTPUT_KOINU)
        return normalized
    }

    private fun validateRawTransactionHex(rawTransactionHex: String, minimumOutputKoinu: Long) {
        validateRawTransactionShape(
            DogecoinHex.decode(rawTransactionHex),
            dogecoinEffectiveStandardOutputKoinu(minimumOutputKoinu)
        )
    }

    private fun validateRawTransactionShape(bytes: ByteArray, minimumOutputKoinu: Long) {
        var offset = 0
        ...
        repeat(outputCount.toInt()) { outputIndex ->
            ...
            require(isStandardDogecoinOutputScript(outputScript)) {
    ...
    private fun isStandardDogecoinOutputScript(script: ByteArray): Boolean {
